...
def setup_seed(seed):  # 2022/01/21 18:20 增加随机种子，使得实验可复现
...
def main():
    ...
    parser.add_argument('--data_path', default='/workspace/Image-Text_Retrieval/datasets',  # default='./data/',
                        help='path to datasets')
    parser.add_argument('--data_name', default='coco',
                        help='{coco,f30k}_precomp')
    parser.add_argument('--vocab_path', default='/workspace/Image-Text_Retrieval/GSMN/vocab',  # default='./vocab/',
                        help='Path to saved vocabulary json files.')

    # ---------------------------------------------------------------#
    parser.add_argument('--seed', default=None, help='default: random')  # 2022/01/21 18:15 增加随机种子
    parser.add_argument('--cuda', default='0', help='default: random')  # 2022/01/21 18:15 增加GPU选取
    # ---------------------------------------------------------------#
    ...
    parser.add_argument('--logger_name', default='./runs/{}_GSMN/log',
                        help='Path to save Tensorboard log.')
    parser.add_argument('--model_name', default='./runs/{}_GSMN/checkpoint',
    ...
    if opt.seed is None:  # 2022/01/21 18:17 增加随机种子为文件命名
        opt.seed = random.randint(0, 100000)
    opt.logger_name = opt.logger_name.format(opt.data_name) + '_' + str(opt.seed)
    opt.model_name = opt.model_name.format(opt.data_name) + '_' + str(opt.seed)
    opt.seed = int(opt.seed)

    opt.data_name = opt.data_name + '_precomp'
    print(opt)

    setup_seed(opt.seed)  # 2022/01/21 18:19 增加随机种子，使得实验可复现

    logging.basicConfig(format='%(asctime)s %(message)s', level=logging.INFO)
    tb_logger.configure(opt.logger_name, flush_secs=5)

    if not os.path.exists(opt.model_name):  # 2022/01/21 18:19 移动文件夹创建部分，在开始部分创建只需判断一次。
        os.mkdir(opt.model_name)

    argsDict = opt.__dict__  # 2022/01/21 10:48 保留设置数据
    with open(opt.model_name + '/setting.txt', 'w') as f:
