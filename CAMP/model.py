...
class SimLoss(nn.Module):
    ...
    def __init__(self, margin=0, measure=False, max_violation=False, inner_dim=0, loss_func="BCE"):
        ...
        if measure == 'cosine':
            ...
            self.sim = CrossAttention(inner_dim, 4, -1).cuda()
        elif measure == 'cross_attention_new':
            self.sim = CrossAttentionNew(inner_dim, 4, -1).cuda()
        elif measure == 'gate_fusion':
            self.sim = GatedFusion(inner_dim, 4, 0.0).cuda()
        elif measure == 'gate_fusion_new':
            self.sim = GatedFusionNew(inner_dim, 4, 0.0).cuda()
        else:
            self.sim = cosine_sim
...
class CAMP(object):
    ...
    def __init__(self, opt):
        ...
        if torch.cuda.is_available():
            ...
            #     self.img_enc.cuda()
            #     self.txt_enc.cuda()
            #     self.img_enc = DDP(self.img_enc)
            #     self.txt_enc = DDP(self.txt_enc)
            if opt.cross_model:
                # if not opt.ddp:
                ...
                # else:
                #     self.criterion.sim.cuda()
                #     self.criterion.sim = DDP(self.criterion.sim)
