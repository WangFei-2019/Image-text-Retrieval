...
def main():
    ...
    # ---------------------------------------------------------------#
    parser.add_argument('--seed', default=None, help='default: random')  # 增加随机种子
    parser.add_argument('--cuda', default='0', help='default: random')  # 增加GPU选取
    parser.add_argument('--ddp', action='store_true', help='default: random')  # DDP
    # ---------------------------------------------------------------#
    parser.add_argument('--config', default='', help='Config path.')
    args = parser.parse_args()

    os.environ["CUDA_VISIBLE_DEVICES"] = args.cuda  # 2022/01/21 18:22 设置使用的GPU

    if args.seed is None:  # 2022/01/20 19:28 增加随机种子为文件命名
        args.seed = random.randint(0, 100000)
    args.seed = int(args.seed)

    setup_seed(args.seed)  # 2022/01/20 增加随机种子，使得实验可复现
    ...
    opt.logger_name = opt.logger_name + '_seed' + str(opt.seed)
    print(opt)
