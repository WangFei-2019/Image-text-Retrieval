...
class LogCollector(object):
    ...
    def __str__(self):
        ...
        """
        s = ''
        for i, (k, v) in enumerate(self.meters.items()):  # 2022/01/21 10:11 .iteritems() 已经弃用，更换为 .items()
            if i > 0:
                s += '  '
            s += k + ' ' + str(v)
        return s
    ...
    def tb_log(self, tb_logger, prefix='', step=None):
        ...
        """
        for k, v in self.meters.items():  # 2022/01/21 10:07 .iteritems() 已经弃用，更换为 .items()
...
if __name__ == '__main__':
    for i in range(9):
        print((i + 2) * 1000)
        evalrank("/workspace/Image-Text_Retrieval/SCAN/runs10/f30k_scan/checkpoint_i-tAVG_" + str(
            (i + 2) * 1000) + "/model_best.pth.tar",
                 data_path='/workspace/Image-Text_Retrieval/datasets', split="test", fold5=False)
