...
def setup_seed(seed):  # 2022/01/21 10:20 增加随机种子，使得实验可复现
...
def main():
    ...
    parser.add_argument('--vocab_path', default='/workspace/Image-Text_Retrieval/SCAN/vocab',  # default='./vocab/',
    ...
    parser.add_argument('--logger_name', default='./runs/{}_scan/log',
                        help='Path to save Tensorboard log.')
    parser.add_argument('--model_name', default='./runs/{}_scan/checkpoint',
    ...
    setup_seed(opt.seed)  # 2022/01/20 10:21 增加随机种子，使得实验可复现

    logging.basicConfig(format='%(asctime)s %(message)s', level=logging.INFO)
    tb_logger.configure(opt.logger_name, flush_secs=5)

    if not os.path.exists(opt.model_name):  # 2022/01/21 10:25 移动文件夹创建部分，在开始部分创建只需判断一次。
