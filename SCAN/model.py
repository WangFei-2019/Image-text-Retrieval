...
class EncoderText(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        if self.use_bi_gru:
            cap_emb = (cap_emb[:, :, :cap_emb.size(2) // 2] + cap_emb[:, :, cap_emb.size(
                2) // 2:]) / 2  # 2022/01/20 23:08 索引中的 / 更换为 //，索引不能为float
...
def func_attention(query, context, opt, smooth, eps=1e-8):
    ...
    attn = nn.Softmax(dim=1)(attn * smooth)  # 2022/01/20 23:17 nn.Softmax()隐式维度默认已经弃用，增加dim
    # --> (batch, queryL, sourceL)
    attn = attn.view(batch_size, queryL, sourceL)
    # --> (batch, sourceL, queryL)
...
class SCAN(object):
    ...
    def forward_loss(self, img_emb, cap_emb, cap_len, **kwargs):
        ...
        self.logger.update('Le', loss.data,
                           img_emb.size(0))  # 2022/01/20 23:13 去掉了loss.data[0]后面的维度取值，python3不再存在一维的tensor
        return loss
